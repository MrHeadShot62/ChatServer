package com.mrheadshot62.server.storage;

/**
 * Created by novak on 07.01.2017.
 */
public class Tables {
    public static final String USER = "users";
    public static final String USER_USER_ID = "user_id";
    public static final String USER_LOGIN = "login";
    public static final String USER_PASS = "pass";
    public static final String USER_FNAME = "fname";
    public static final String USER_LNAME = "lname";
    public static final String USER_NICK = "nick_name";
    public static final String USER_AGE = "age";
    public static final String USER_COUNTRY = "country";
    public static final String USER_CITY = "city";
    public static final String USER_RATING = "rating";
    public static final String USER_PHOTOS = "photos";
    public static final String USER_PROFILEPHOTO = "profile_photo";
    public static final String USER_LASTONLINE = "last_online";
    public static final String USER_EMAIL = "email";
    public static final String USER_FRIENDS = "friends";
    public static final String USER_SESSION = "session";
    public static final String USER_PERMISSIONLVL = "permissionLevel";
    public static final String USER_LAST_EDIT = "last_edit";
    public static final String USER_COUNT_PHOTO = "last_edit";

    public static final String IMAGE = "images";
    public static final String IMAGE_NAME = "name";
    public static final String IMAGE_PATH = "path";
    public static final String IMAGE_AUTHOR = "author";
    public static final String IMAGE_TIME = "time";
    public static final String IMAGE_REPORT = "report";
    public static final String IMAGE_LIKES = "likes";

}
